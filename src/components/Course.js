...
const Course = ({
  ...
  const getStars = (rate, isVotable) => {
    ...
    const faRegStars = Array.from(
      { length: 5 - faStars.length },
      (_, index) => (
        <FaRegStar
          key={"regStar" + index}
          color="gold"
          onMouseOver={() =>
            isVotable && setUserVote(faStars.length + index + 1)
          }
          onMouseOut={() => isVotable && setUserVote(null)}
  ...
  const addComment = (e) => {
    ...
    fetch(`http://localhost:8080/courses/${id}/comments`, {
      ...
      .then((newCom) => {
        ...
        fetch(
          `http://localhost:8080/courses/${id}/comments/${newCom.commentId}/img`,
          {
            method: "post",
            body: formData,
          }
          ...
          .then(() => {
            ...
            copyCourse.course.comments[index] = {
              ...newCom,
              image: {
                filePath: `http://${process.env.REACT_APP_URL}:8080/courses/${id}/comments/${newCom.commentId}/img`,
              },
            };
        ...
        <>
          {isBought ? (
            <video
              controls
              autoPlay
              src={`http://localhost:8080/courses/${id}/video`}
            />
          ) : (
            <img
              ...
              src={
                courseModel.course.image
                  ? `http://localhost:8080/courses/${id}/img`
                  : "/assets/react_js.jpg"
              }
            />
          )}
